...
CTable CTable::operator / (int iPace)
{
	...
	CTable c_table_new(s_name + " modified", i_new_tableLen);
	int *pi_new_table = new int[i_new_tableLen];
	for (int i = 0; i < i_new_tableLen; i++)
		pi_new_table[i] = pi_table[i * iPace];
	c_table_new.pi_table = pi_new_table;
	c_table_new.i_tableLen = i_new_tableLen;
	return move(c_table_new);
}
void CTable::vMoveSemantic(CTable & pcOther)
{
...
void CTable::v_copy(const CTable & pcOther)
{
	i_tableLen = pcOther.i_tableLen;
	pi_table = new int[i_tableLen];
	for (int i = 0; i < i_tableLen; i++) pi_table[i] = pcOther.pi_table[i];
